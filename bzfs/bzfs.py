...
""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument(
        ...
        #       "a) a non-negative integer representing a Unix time in seconds. Example: 1728109805\n\n"
        ...
        #       "Examples: '45min', '2h', '90 days', '12w'.\n\n"
        #       "Note: This option compares the specified time against the standard ZFS 'creation' time property of the "
        #       "snapshot (which is a Unix time in integer seconds), rather than against a timestamp that may be part of "
        #       "the snapshot name. See https://openzfs.github.io/openzfs-docs/man/master/7/zfsprops.7.html#creation"
        #       "\n\n"))
...
TimeRange = Optional[Tuple[int, int]]  # Type alias
...
class Params:
    def __init__(
        ...
        self.snapshot_time_range: TimeRange = self.get_snapshot_time_range(args)
    ...
    @staticmethod
    def get_snapshot_time_range(args: argparse.Namespace) -> TimeRange:
        def unix_time_in_seconds(time_spec: Union[timedelta, int], default: int) -> int:
        ...
        lo = unix_time_in_seconds(args.include_snapshot_from_time, default=0)
        hi = unix_time_in_seconds(args.include_snapshot_to_time, default=1000000000000)  # 33658-09-27 ~ infinity
...
class Job:
    ...
    def filter_snapshots_by_creation_time(
        self, snapshots: List[str], snapshot_time_range: TimeRange, bookmark_time_range: TimeRange
    ) -> List[str]:
        p, log = self.params, self.params.log
        is_debug = log.isEnabledFor(log_debug)
        results = []
        ...
        if values.isdigit():
            ...
            try:
                ...
                try:  # If it's not a duration, try parsing as an ISO 8601 datetime
                    ...
                    parser.error(f"{option_string}: Invalid time, duration, or Unix time: {values}")
